...
from shared import PagerDutyClient, SlackClient
...
def trigger_incident_for_spike(spike: dict, pd_client: PagerDutyClient, slack_client: SlackClient, routing_key: str) -> dict:
    ...
    if pd_result.get('success'):
        ...
        slack_client.post_message(slack_msg, SLACK_CHANNEL)
...
def lambda_handler(event, context):
    ...
    if trigger_pagerduty and spikes and PAGERDUTY_ROUTING_KEY:
        pd_results = []
        for spike in spikes[:1]:
            pd_result = trigger_incident_for_spike(spike, pagerduty, slack, PAGERDUTY_ROUTING_KEY)
